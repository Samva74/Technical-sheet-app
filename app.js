...
function fillPreviewGallery(container, documents){
    if(!container) return;
    ...
    /*
     * Le nombre de fichiers est transmis au CSS.
     * La grille pourra ainsi changer automatiquement.
     */
    container.dataset.count = documents.length;

    /*
     * Classes utiles pour les cas particuliers.
     */
    container.classList.toggle('is-empty', documents.length === 0);
    container.classList.toggle('is-single', documents.length === 1);
}

function configureGallery(container, documents){
...
function renderPreviews(){
    ...
    const clientDocuments = d.documents.filter(
        document => document.context === 'client'
    );

    const productDocuments = d.documents.filter(
        document => document.context === 'product'
    );

    const outerboxDocuments = d.documents.filter(
        document => document.context === 'outerbox'
    );
    ...
    all('[data-preview-component]').forEach(container => {

        const componentDocuments = d.documents.filter(
            document =>
                document.context === 'component' &&
                document.contextId ===
                    container.dataset.previewComponent
        );
    ...
    all('[data-preview-mark]').forEach(container => {

        const markingDocuments = d.documents.filter(
            document =>
                document.context === 'marking' &&
                document.contextId ===
                    container.dataset.previewMark
        );
    ...
    all('.remove-doc').forEach(button => {

        button.onclick = () =>
            removeDoc(button.dataset.doc);
    });
}
  all('[data-preview-component]').forEach(box=>box.innerHTML=d.documents.filter(x=>x.context==='component'&&x.contextId===box.dataset.previewComponent).map(previewHtml).join(''));all('[data-preview-mark]').forEach(box=>box.innerHTML=d.documents.filter(x=>x.context==='marking'&&x.contextId===box.dataset.previewMark).map(previewHtml).join(''));all('.remove-doc').forEach(b=>b.onclick=()=>removeDoc(b.dataset.doc))}function renderOuterboxConditional(){cavityDimensionsWrap.hidden=d.outerbox.leaflet!=='Oui / Yes';drawerDescriptionWrap.hidden=d.outerbox.drawer!=='Oui / Yes'}function removeDoc(id){d.documents=d.documents.filter(x=>x.id!==id);renderPreviews();renderDocuments();dirty()}
