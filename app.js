...
function renderAll(){all('[data-p]').forEach(x=>x.value=get(d,x.dataset.p)??'');renderQuantities();renderComponents();renderMarkings(););renderDocuments();renderOuterboxConditional();renderSummary();validateReference();applyTranslations()}
...
function validateReference(){let ok=validRef(),r=(d.client.sourceReference||'').trim();refText.textContent=ok?r:tr('waiting');refText.classList.toggle('waiting',!ok);requiredMsg.hidden=ok;
...
productPreview.dataset.count =
    productFiles.length;

productPreview.classList.toggle(
    'is-single',
...
function contextLabel(c){return {client:'Client & Projet',product:'Produit',component:tr('components'),marking:tr('markings'),outerbox:tr('outerbox'),documents:'Documents'}[c]||c}
function formatSize(n){return n>1048576?(n/1048576).toFixed(1)+' Mo':Math.round(n/1024)+' Ko'}
...
function download(bytes,name,type){let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([bytes],{type}));a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function importJson(e){let f=e.target.files[0];if(!f)return;let r=new FileReader();r.onload=()=>{try{d=JSON.parse(r.result);if(!d.components?.length)d.components=[newComponent()];if(!d.markings?.length)d.markings=[newMarking()];renderAll();show(0);adminDialog.close()}catch{alert('JSON invalide')}};r.readAsText(f)}
...
function b64Bytes(dataUrl){let b=atob(dataUrl.split(',')[1]||''),u=new Uint8Array(b.length);for(let i=0;i<b.length;i++)u[i]=b.charCodeAt(i);return u}
function crc32(u){let c=-1;for(let n of u){c^=n;for(let k=0;k<8;k++)c=(c>>>1)^((c&1)?0xEDB88320:0)}return(c^-1)>>>0}
function dosTime(d=new Date()){return((d.getHours()<<11)|(d.getMinutes()<<5)|(d.getSeconds()>>1))&65535}
function dosDate(d=new Date()){return(((d.getFullYear()-1980)<<9)|((d.getMonth()+1)<<5)|d.getDate())&65535}
...
let currentLang=localStorage.getItem('ftLang')||'fr';
function tr(k){return DICT[currentLang]?.[k]||DICT.fr[k]||k}
function trText(v){return v}
