...
function validRef(){return !!(d.client.sourceReference||'').trim()}function safeName(){return (d.client.sourceReference||'Technical-Sheet').trim().replace(/[\\/:*?"<>|]+/g,'-')}function validateReference(){let ok=validRef(),r=(d.client.sourceReference||'').trim();refText.textContent=ok?r:tr('waiting');refText.classList.toggle('waiting',!ok);requiredMsg.hidden=ok;[saveBtn,pdfBtn,zipBtn].forEach(b=>b.disabled=!ok)}function saveUser(){if(!validRef())return;localStorage.setItem('ftDraftV10',JSON.stringify(d));state.style.color='#2f9149'}function dirty(){state.style.color='#f47738';validateReference()}
...
/* =========================================================
   CARTE 07 : AJUSTEMENT AUTOMATIQUE DES VIGNETTES
   ========================================================= */

function fitDocumentThumbnails(*{
    const documentList = documen*.getElementById("documentList");

*   if(!documentList) return;

    *onst visibleGroups = Array
       *.from(documentList.querySelectorAl*(".document-group"))
        .filt*r(group => {
            return !g*oup.hidden &&
                   g*tComputedStyle(group).display !== *none";
        });

    visibleGro*ps.forEach(group => {
        const grid =
            group.querySelector(".document-group-grid");

        if(!grid) return;

        const cards =
            Array.from(grid.querySelectorAll(".document-card"));
        ...
        /*
         * Si la carte 07 n'est pas encore visible,
         * les dimensions peuvent être temporairement nulles.
         */
        ...
        /*
         * Teste toutes les répartitions possibles afin de
         * trouver les vignettes les plus grandes pouvant
         * rentrer dans la largeur et la hauteur disponibles.
         */
        for(let columns = 1; col*mns <= count; columns++){
        *   const rows = Math.ceil(count / *olumns);
            ...
            /*
             * Le score privilégie une vignette équilibrée
             * et assez grande pour afficher le document.
             */
            const score = Math.m*n(cardWidth, cardHeight);
   ...
   *            bestRows = rows;
     *      }
        ...
        grid.st*le.gridTemplateColumns =
         *  `repeat(${bestColumns}, minmax(0, 1fr))`;

        grid.style.gridTemplateRows =
            `repeat(${bestRows}, minmax(0, 1fr))`;
    });
}


/*
 * Lance le calcul après que le navigateur a terminé
 * d'afficher la carte ou le filtre sélectionné.
 */
function scheduleDocumentThumbn*ilFit(){
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            fitDocumentThumbnails();
        });
    });
}
...
/*
 * Recalcul après un clic sur :
 * Tous, Client & Projet, Produit, Composants, etc.
 */
document.addEventListener("click", event => {
    if(event.target.closest(".doc-filters button")){
        scheduleDocumentThumbnailFit();
    }
});


/*
 * Recalcul lorsqu'un fichier est ajouté, supprimé
 * ou lorsqu'une catégorie devient visible ou masquée.
 */
const documentListObserverTarget =
    document.getElementById("documentList");

if(documentListObserverTarget){
    const documentObserver = new MutationObserver(() => {
        scheduleDocumentThumbnailFit();
    });

    documentObserver.observe(
        documentListObserverTarget,
        {
            childList:true,
            subtree:true,
            attributes:true,
...
/*
 * Recalcul lorsque la fenêtre change de dimensions.
 */
...
/*
 * Premier calcul au chargement de l'application.
 */
